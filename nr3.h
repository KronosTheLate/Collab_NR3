...
public:
	...
	~NRvector();
	friend ostream& operator<<(ostream& os, const NRvector<double>& nrvec);
};

ostream& operator<<(ostream& os, const NRvector<double>& nrvec)
{   
    cout << "NRvector of length " << nrvec.size() <<":\n";
...
public:
	...
	~NRmatrix();
    friend ostream& operator<<(ostream& os, const NRmatrix<double>& nrmat);
};

ostream& operator<<(ostream& os, const NRmatrix<double>& nrmat)
{
    cout << nrmat.nrows() << " x " << nrmat.ncols() << "  matrix:\n";    
    for (int i = 0; i<nrmat.nrows(); i++){
        for (int j = 0; j<nrmat.ncols(); j++){
        cout << nrmat[i][j] << "\t";
        }
